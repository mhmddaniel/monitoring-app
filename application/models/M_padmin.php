...
class M_padmin extends CI_Model{
	...
	function save_anggaran_lap($anggaran_id,$dayaserap,$pagu,$newanggaran){
		$hsl=$this->db->query("INSERT INTO anggaran_lap VALUES(null,'$anggaran_id','$dayaserap','$pagu','$newanggaran',NOW())");
		return $hsl;
	}

	function get_anggaran_lap_by_kode($kode){
		$hsl=$this->db->query("SELECT * FROM anggaran_lap where anggaran_id='$kode' order by al_tanggal desc");
		return $hsl;
	}
	...
	function sum_anggaran(){
		$hsl=$this->db->query("SELECT Sum(anggaran_pagu) as sumanggaran from anggaran");
		return $hsl->row_array();
	}
	...
	function get_all_proyek(){
		$hsl=$this->db->query("SELECT * FROM proyek left join proyek_bagian on proyek.proyek_id=proyek_bagian.pb_proyek_id inner join koordinat on proyek.proyek_koordinat_id=koordinat.koordinat_id inner join user on proyek.user_id=user.user_id group by proyek.proyek_id");
		return $hsl;
	}
	...
	function save_serap($gtserap,$gpersen,$gbulan,$gfisik){
		$hsl=$this->db->query("INSERT INTO serapan VALUES(null,'$gtserap','$gpersen','$gbulan','$gfisik')");
		return $hsl;
	}

	function update_serap($gtserap,$gpersen,$gbulan,$gfisik){
		$hsl=$this->db->query("UPDATE serapan set serapan_target='$gtserap',serapan_persen='$gpersen',serapan_fisik='$gfisik' where serapan_bulan='$gbulan'");
		return $hsl;
	}

	function get_all_proyek_by_bagian($phid){
		$hsl=$this->db->query("SELECT * FROM proyek left join proyek_bagian on proyek.proyek_id=proyek_bagian.pb_proyek_id inner join user on proyek.user_id=user.user_id inner join koordinat on proyek.proyek_koordinat_id=koordinat.koordinat_id where proyek.ph_id='$phid' group by proyek.proyek_id ");
		return $hsl;
	}
	...
	function get_chart_rt_all(){
		$hsl=$this->db->query("SELECT proyek_bagian.*,DATE_FORMAT(pb_tanggal_prog,'%d %M %Y') AS tanggal FROM proyek_bagian where pb_jenis='fisik' ORDER BY proyek_bagian.pb_id  asc");
		return $hsl;
	}
	...
	function get_chart_rt_all_by_bagian($bagian){
		$hsl=$this->db->query("SELECT DISTINCT *,DATE_FORMAT( proyek_bagian.pb_tanggal_prog,'%d %M %Y') AS tanggal FROM proyek_bagian left join proyek on proyek_bagian.pb_proyek_id=proyek.proyek_id inner join proyek_head on proyek.ph_id=proyek_head.ph_id where proyek_head.ph_bidang='$bagian' and proyek_bagian.pb_jenis='fisik' ORDER BY proyek_bagian.pb_id asc");
		return $hsl;
	}
	...
	function diffdateplus(){
		$hsl=$this->db->query("SELECT count(*) as countkerja from proyek_bagian inner join proyek on proyek.proyek_id=proyek_bagian.pb_proyek_id where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='fisik' group by pb_proyek_id) and proyek_bagian.pb_real!=100 AND proyek_awal_kontrak<now() and proyek_bagian.pb_real>0");
		return $hsl;
	}*/


	function tesdikerjakan(){
		$hsl=$this->db->query("SELECT * from proyek left join proyek_bagian on proyek.proyek_id=proyek_bagian.pb_proyek_id where proyek.proyek_awal_kontrak is not null  GROUP by proyek.proyek_id");
		return $hsl;
	}

	function tesdikerjakan_bagian($phid){
		$hsl=$this->db->query("SELECT * from proyek left join proyek_bagian on proyek.proyek_id=proyek_bagian.pb_proyek_id where proyek.proyek_awal_kontrak is not null and proyek.ph_id='$phid'  GROUP by proyek.proyek_id");
		return $hsl;
	}

	function cekdikerjakan($proyek_id){
		$hsl=$this->db->query("SELECT max(pb_real) as maxpb from proyek_bagian where proyek_bagian.pb_proyek_id='$proyek_id'");
		return $hsl->row_array();
	}

	function diffdateplus(){
		$hsl=$this->db->query("SELECT count(*) as countkerja from proyek_bagian inner join proyek on proyek.proyek_id=proyek_bagian.pb_proyek_id where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='fisik' and proyek_awal_kontrak is not null and pb_real<100 group by pb_proyek_id)");
		return $hsl;
	}
	function diffdatemin(){
		$hsl=$this->db->query("SELECT count(DISTINCT  proyek.proyek_id) as countkerja FROM proyek left join proyek_bagian on proyek.proyek_id=proyek_bagian.pb_proyek_id where  proyek.proyek_awal_kontrak is null ");
		return $hsl;
	}
	...
	function diffdateplus_by_kode($phid){
		$hsl=$this->db->query("SELECT count(*) as countkerja from proyek_bagian inner join proyek on proyek.proyek_id=proyek_bagian.pb_proyek_id where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='fisik'  group by pb_proyek_id) ");
		return $hsl;
	}
	...
	function countjum(){
		$hsl=$this->db->query("SELECT MONTHNAME(CONCAT('2018-', m.month, '-01'))  proyek_bulan,
			coalesce(s.count, 0) countsech,
			coalesce(e.count, 0) countawal
			FROM (SELECT 1 month
			UNION ALL
			SELECT 2 month
			UNION ALL
			...
			UNION ALL
			SELECT 12 month) m
			LEFT JOIN (SELECT month(n.proyek_sech_awal) month,
			count(*) count
			FROM proyek n
			GROUP BY month(n.proyek_sech_awal)) s
			ON s.month = m.month
			LEFT JOIN (SELECT month(n.proyek_awal_kontrak) month,
			count(*) count
			FROM proyek n
			GROUP BY month(n.proyek_awal_kontrak)) e
			ON e.month = m.month
			ORDER BY m.month");
		return $hsl;
	}
	function countjum_by_kode($phid){
		$hsl=$this->db->query("SELECT MONTHNAME(CONCAT('2018-', m.month, '-01'))  proyek_bulan,
			...
			LEFT JOIN (SELECT month(n.proyek_sech_awal) month,
			count(*) count
			FROM proyek n where ph_id='$phid'
			GROUP BY month(n.proyek_sech_awal)) s
			ON s.month = m.month
			LEFT JOIN (SELECT month(n.proyek_awal_kontrak) month,
			count(*) count
	...
	function sum_sisa_by_kode($phid){
		$hsl=$this->db->query("SELECT distinct sum(pb_sisa_anggaran) AS sumsisa FROM proyek_bagian,proyek where proyek.ph_id='$phid' && proyek_bagian.pb_proyek_id=proyek.proyek_id ");
		return $hsl;
	}
	function sum_proyek(){
	...
	function sum_realisasi_by_ph($ph_id){
		$hsl=$this->db->query("SELECT * from proyek_bagian left join proyek on proyek_bagian.pb_proyek_id=proyek.proyek_id left join proyek_head on proyek.ph_id=proyek_head.ph_id where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='keuangan' group by pb_proyek_id) and proyek.ph_id='$ph_id'");
		return $hsl;
	}
	...
	function sum_realisasi_proyek($proyekid){
		$hsl=$this->db->query("SELECT * from proyek_bagian where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='keuangan' AND pb_proyek_id='$proyekid' group by pb_proyek_id)");
		return $hsl->row_array();
	}

	function get_xel(){
		$hsl=$this->db->query("SELECT * from proyek_bagian left join proyek on proyek_bagian.pb_proyek_id=proyek.proyek_id left join proyek_head on proyek.ph_id=proyek_head.ph_id left join user on proyek.user_id=user.user_id where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='keuangan' group by pb_proyek_id) GROUP by proyek_head.ph_id  order by proyek_head.ph_kredit asc");
		return $hsl;
	}
	function get_xel_bagian($bagian){
		$hsl=$this->db->query("SELECT * from proyek_bagian left join proyek on proyek_bagian.pb_proyek_id=proyek.proyek_id left join proyek_head on proyek.ph_id=proyek_head.ph_id left join user on proyek.user_id=user.user_id where pb_id in (select max(pb_id) from proyek_bagian where proyek_bagian.pb_jenis='keuangan' group by pb_proyek_id) AND ph_bidang='$bagian' GROUP by proyek_head.ph_id  order by proyek_head.ph_kredit asc");
		return $hsl;
	}
...
/*
	function get_all_pn_by_kode($kode){
		$this->db->select('*');
		$this->db->from('pekerja a');
		$this->db->join('proyek b','a.pekerja_id=b.pekerja_id','left');
...
*/
	...
	function get_penanggung_jawab(){
		$hsl=$this->db->query("SELECT * FROM pekerja");
		return $hsl;
	}
	...
	function get_pekerja_by_pekerja_id($pekerja_id){
		$hsl=$this->db->query("SELECT * FROM proyek inner join pekerja on proyek.pekerja_id=pekerja.pekerja_id where proyek.pekerja_id='$pekerja_id'");
		return $hsl;
	}
	...
	function get_max_proyek_id(){
		$hsl=$this->db->query("SELECT * FROM proyek order by proyek_id desc limit 1");
		return $hsl->row_array();
	}
	function get_proyek(){
	...
	function save_user($user_id,$username,$password,$user_nama,$user_email,$user_telp,$user_bagian,$user_level){
		$hsl=$this->db->query("INSERT INTO user VALUES('$user_id','$username',md5('$password'),'$user_nama','$user_email','$user_telp','$user_bagian','$user_level','user_blank.png')");
		return $hsl;
	}
	function get_user_by_id_pptk(){
		$hsl=$this->db->query("SELECT * from user where user_level='pptk'");
		return $hsl;
	}
...
*/
	...
	function save_proyek($numproyek,$user_id,$numkor,$xnama,$year,$pagu,$xjenis,$xvolume,$xsatuan,$phid){
		$hsl=$this->db->query("INSERT INTO proyek (proyek_id,ph_id,proyek_koordinat_id,user_id,proyek_nama,proyek_tahun,proyek_keuangan,proyek_pagu,proyek_jenis,proyek_volume,proyek_satuan,last_update) VALUES ('$numproyek','$phid','$numkor','$user_id','$xnama','$year','0','$pagu','$xjenis','$xvolume','$xsatuan',NOW())");
		return $hsl;
	}
	function save_ph($judulph,$ph_bidang,$kreditph){
		$hsl=$this->db->query("INSERT INTO proyek_head (ph_judul,ph_bidang,ph_kredit) VALUES ('$judulph','$ph_bidang','$kreditph')");
		return $hsl;
	}
	function update_ph($phid,$judulph,$ph_bidang,$kreditph){
		$hsl=$this->db->query("UPDATE proyek_head set ph_judul='$judulph',ph_bidang='$ph_bidang',ph_kredit='$kreditph' where ph_id='$phid'");
		return $hsl;
	}
	...
	function get_last_id_pekerja(){
		$hsl=$this->db->query("SELECT max(pekerja_id) as pekerja_id from pekerja");
		return $hsl->row_array();
	}
	function update_pekerja_proyek($proyek_id,$pekerja_id){
		$hsl=$this->db->query("UPDATE proyek set pekerja_id='$pekerja_id' where proyek_id='$proyek_id'");
		return $hsl;
	}

	function update_proyek_n($proyek_id,$user_id,$numkor,$xnama,$year,$pagu,$xjenis,$xvolume,$xsatuan){
		$hsl=$this->db->query("UPDATE proyek set user_id='$numkor',proyek_koordinat_id='$user_id',proyek_nama='$xnama',proyek_tahun='$year',proyek_pagu='$pagu',proyek_jenis='$xjenis',proyek_volume='$xvolume',proyek_satuan='$xsatuan' where proyek_id='$proyek_id'");
		return $hsl;
	}
	function update_proyek($proyek_id,$user_id,$numkor,$xnama,$year,$pagu,$xjenis,$xvolume,$xsatuan){
		$hsl=$this->db->query("UPDATE proyek set user_id='$user_id',proyek_koordinat_id='$numkor',proyek_nama='$xnama',proyek_tahun='$year',proyek_pagu='$pagu',proyek_jenis='$xjenis',proyek_volume='$xvolume',proyek_satuan='$xsatuan' where proyek_id='$proyek_id'");
		return $hsl;
	}
	...
	function save_pekerja($xpekjenis,$xnamdirek,$xteldirek,$xnamaperus,$xalaperus,$xtelkant){
		$hsl=$this->db->query("INSERT INTO pekerja (pekerja_jenis,pekerja_nama_direktur,pekerja_tel_direktur,pekerja_nama_perusahaan,pekerja_alamat_perusahaan,pekerja_tel_kantor) VALUES ('$xpekjenis','$xnamdirek','$xteldirek','$xnamaperus','$xalaperus','$xtelkant')");
		return $hsl;
	}


	function update_pn($xnip,$xpekjenis,$xnamdirek,$xteldirek,$xnamaperus,$xalaperus,$xtelkant){
		$hsl=$this->db->query("update pekerja SET pekerja_jenis='$xpekjenis',pekerja_nama_direktur='$xnamdirek',pekerja_tel_direktur='$xteldirek',pekerja_nama_perusahaan='$xnamaperus',pekerja_alamat_perusahaan='$xalaperus',pekerja_tel_kantor='$xtelkant' where pekerja_id='$xnip'");
		return $hsl;
	}
	...
	function get_penannggung_jawab($kode){	
		$this->db->select('*');
		$this->db->from('proyek a');
		$this->db->join('pekerja b','a.pekerja_id=b.pekerja_id','inner');
	...
	function get_detail_proyek_by_kode($kode){
		...
		$this->db->join('koordinat c','a.proyek_koordinat_id=c.koordinat_id','inner');
		$this->db->join('user d','a.user_id=d.user_id','inner');
		$this->db->where('a.proyek_id',$kode);
